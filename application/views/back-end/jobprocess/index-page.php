                     ...
                     <div class="col-md-4 text-right">
                        <div class="card-footer bg-transparent" style="margin-top: -15px;">
                            <div class="text-center">
                                <a href="<?= site_url('admin/jobprocess/create') ?>" class="btn btn-outline-success btn-sm align-middle me-2" title="Job Process" style="float: right;">
                     ...
                     <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                        <thead class="table-light">
                           <tr>
                              ...
                              <th class="align-middle">Job Process Name</th>
                        ...
                        <tbody>
                           ...
                           <tr>
                              ...
                              <td><?= $data->job_process_name ?></td>
                               <td>
                                 <input type="checkbox" class="jobprocess-ststus" data-id="<?= $data->job_process_id ?>"<?= $data->job_process_status =="1" ? "checked" : "" ?> id="jobprocess<?= $key + 1 ?>" switch="info"/>
                                 <label for="jobprocess<?= $key + 1 ?>" data-on-label="Yes" data-off-label="No"></label>
                              </td>
                              <td>
                                 <div class="d-flex gap-1">
                                    <a href="<?= site_url('admin/jobprocess/edit/'.$data->job_process_id) ?>" class="btn btn-outline-secondary btn-sm"><i class="mdi mdi-pencil font-size-13"></i></a>
                                 </div>
                              </td>
                           </tr>
                           <?php } ?>
...
<script type="text/javascript">
   ...
   $(document).on('change','.jobprocess-ststus',function(){
    ...
    if (_this.prop('checked') == true){ 
      ...
      $.ajax({
        method: "POST",
        url: "<?= base_url('admin/jobprocess/jobprocessstatus') ?>",
        data: { job_process_id: _data_id, status: _data_status }
