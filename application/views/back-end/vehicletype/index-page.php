      ...
      <div class="row">
         <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
               <h4 class="mb-sm-0 font-size-18">Vehicle Type</h4>
               ...
               <div class="card-body">
                     ...
                     <div class="col-md-4 text-right">
                        <div class="card-footer bg-transparent" style="margin-top: -15px;">
                            <div class="text-center">
                                <a href="<?= site_url('admin/vehicletype/create') ?>" class="btn btn-outline-success btn-sm align-middle me-2" title="New Vehicle" style="float: right;">
                                    <i class="fas fa-plus"></i> Vehicle Type
                                </a>
                            </div>
                        </div>
                    </div>
                  ...
                  <div class="table-responsive">
                     <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                        ...
                        <tbody>
                           <?php foreach($vehicle_type->result() as $key => $data) { ?>
                           <tr>
                              <td><a href="javascript: void(0);" class="text-body fw-bold">#<?= $key + 1 ?></a> </td>
                              <td><?= $data->vehicle_type_name ?></td>
                               <td>
                                 <input type="checkbox" class="vehicle-ststus" data-id="<?= $data->vehicle_type_id ?>"  <?= $data->vehicle_type_status =="1" ? "checked" : "" ?> id="vehicle<?= $key + 1 ?>" switch="info"/>
                                 <label for="vehicle<?= $key + 1 ?>" data-on-label="Yes" data-off-label="No"></label>
                              </td>
                              <td>
                                 <div class="d-flex gap-3">
                                    <a href="<?= site_url('admin/vehicletype/edit/'.$data->vehicle_type_id) ?>" class="btn btn-outline-secondary btn-sm"><i class="mdi mdi-pencil font-size-13"></i></a>
                                 </div>
                              </td>
                           </tr>
                           <?php } ?>
...
<script type="text/javascript">
   ...
   $(document).on('change','.vehicle-ststus',function(){
    ...
    if (_this.prop('checked') == true){ 
      ...
      $.ajax({
        method: "POST",
        url: "<?= base_url('admin/vehicletype/vehiclestatus') ?>",
        data: { vehicle_type_id: _data_id, status: _data_status }
